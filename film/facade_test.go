...
// GENERATED BY AI
func TestFilmFacade(t *testing.T) {
	testApp := fxtest.New(
		t,
		...
		fx.Invoke(func(facade Facade) {
			t.Run("Create and Retrieve Single Film", func(t *testing.T) {
				err := facade.CreateFilm(FilmDTO{Title: "Test Movie 1"})
				if err != nil {
					t.Fatalf("Failed to create film: %v", err)
				}

				film, err := facade.GetFilmById(1)
				if err != nil {
			...
			t.Run("Create Multiple Films", func(t *testing.T) {
				...
				allFilms, err := facade.GetAllFilms()
				if err != nil {
			...
			t.Run("Get Film By ID", func(t *testing.T) {
				...
				for _, tt := range tests {
					t.Run(tt.name, func(t *testing.T) {
						film, err := facade.GetFilmById(tt.id)
