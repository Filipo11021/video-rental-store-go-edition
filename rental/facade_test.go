...
type mockFilmFacade struct{}

func (m *mockFilmFacade) CreateFilm(filmDTO film.FilmDTO) error { return nil }
func (m *mockFilmFacade) GetAllFilms() ([]film.FilmDTO, error)  { return nil, nil }
func (m *mockFilmFacade) GetFilmById(id int) (*film.FilmDTO, error) {
...
func newMockFilmFacade() film.Facade {
	return &mockFilmFacade{}
}

func TestRentalFacade(t *testing.T) {
	testApp := fxtest.New(
		t,
		fx.Provide(newMemoryRentalRepo),
		fx.Provide(newPriceCalculator),
		fx.Provide(newMockFilmFacade),
		fx.Provide(newFacade),
		fx.Invoke(func(facade Facade) {
			t.Run("Create and Retrieve Rental", func(t *testing.T) {
				err := facade.CreateRental(CreateRentalDTO{
					FilmID: 1,
					Days:   3,
				})
				if err != nil {
				...
				rental, err := facade.GetRentalById(1)
				if err != nil {
			...
			t.Run("Get All Rentals", func(t *testing.T) {
				rentals, err := facade.GetAllRentals()
				if err != nil {
			...
			t.Run("Get Rentals By Film ID", func(t *testing.T) {
				rentals, err := facade.GetRentalsByFilmId(1)
				if err != nil {
			...
			t.Run("Non-existent Rental", func(t *testing.T) {
				rental, err := facade.GetRentalById(999)
				if err != nil {
			...
			t.Run("Calculate Price for Different Film Types", func(t *testing.T) {
				...
				for _, tt := range tests {
					t.Run(tt.name, func(t *testing.T) {
						price, err := facade.CalculatePrice(tt.filmId, tt.days)
						if err != nil {
			...
			t.Run("Calculate Late Charges", func(t *testing.T) {
				...
				for _, tt := range tests {
					t.Run(tt.name, func(t *testing.T) {
						charge, err := facade.CalculateLateCharge(tt.filmId, tt.extraDays)
						if err != nil {
							t.Fatalf("Unexpected error: %v", err)
						}
